...
def cerrar_sesion_participante():
    participante_actual = st.session_state.get("participante_actual", "")
    if participante_actual:
        limpiar_borrador_pronosticos_sesion(participante_actual)
    st.session_state.participante_actual = ""
    st.session_state.participante_autenticado = False
...
def obtener_clave_borrador_pronosticos(participante):
    return f"borrador_pronosticos_{str(participante).strip()}"


def obtener_borrador_pronosticos_sesion(participante, participante_data=None):
    clave_borrador = obtener_clave_borrador_pronosticos(participante)

    if clave_borrador not in st.session_state or not isinstance(st.session_state.get(clave_borrador), dict):
        st.session_state[clave_borrador] = {}

    borrador = st.session_state[clave_borrador]

    if not borrador and participante_data is not None:
        for pronostico in participante_data.get("pronosticos_guardados", []):
            try:
                borrador[int(pronostico["id"])] = {
                    "id": int(pronostico["id"]),
                    "marcador_local": int(pronostico["marcador_local"]),
                    "marcador_visitante": int(pronostico["marcador_visitante"])
                }
            except Exception:
                continue
...
def limpiar_borrador_pronosticos_sesion(participante=None, ids_partidos=None):
    participante_objetivo = participante or st.session_state.get("participante_actual", "")
    if not participante_objetivo:
        return

    clave_borrador = obtener_clave_borrador_pronosticos(participante_objetivo)
    if clave_borrador not in st.session_state:
        return

    if ids_partidos is None:
        st.session_state.pop(clave_borrador, None)
        return

    borrador = st.session_state.get(clave_borrador, {})
    ids_a_quitar = {int(x) for x in ids_partidos}
    for partido_id in ids_a_quitar:
        borrador.pop(int(partido_id), None)
...
def guardar_pronosticos_fase(participante_data, pronosticos_fase):
    ...
    participante_actual = st.session_state.get("participante_actual", "")
    if participante_actual:
        borrador = obtener_borrador_pronosticos_sesion(participante_actual, participante_data)
        for pronostico in pronosticos_fase:
            try:
                borrador[int(pronostico["id"])] = {
                    "id": int(pronostico["id"]),
                    "marcador_local": int(pronostico["marcador_local"]),
        ...
        if st.session_state.participante_autenticado:
            ...
            if participante_data is not None:
                ...
                if fase_seleccionada == "Fase de grupos":
                    st.info("Puedes cambiar entre grupos sin perder lo capturado mientras mantengas la sesión abierta. Solo se guarda en Google Sheets cuando presionas el botón de guardar borrador.")
                ...
                borrador_pronosticos = obtener_borrador_pronosticos_sesion(
                    st.session_state.participante_actual,
                    participante_data
                )
                ...
                for _, p in partidos_fase.iterrows():
                    ...
                    pronostico_borrador = borrador_pronosticos.get(int(p["id"]))
                    pronostico_base = pronostico_borrador or pronostico_existente

                    g1_default = int(pronostico_base["marcador_local"]) if pronostico_base else 0
...
"
                    ...
                    borrador_pronosticos[int(p["id"])] = pronostico_actual
                    pronosticos_temporales.append(pronostico_actual)
